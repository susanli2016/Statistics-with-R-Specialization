...
```{r describe-sample}
ggplot(aes(x=area), data=samp) + geom_histogram(bins=50)
...
cis_99 <- ames %>% 
  rep_sample_n(size=n, reps=50, replace=TRUE) %>% 
  summarise(lower=mean(area)-z_critical_99*(sd(area)/sqrt(n)), upper=mean(area) + z_critical_99*(sd(area)/sqrt(n)))
...
```{r}
cis_99_df <- data.frame(id=c(1:50, 1:50), 
                        bounds = c(cis_99$lower, cis_99$upper),
                        capture_mu=c(cis_99$capture_mu, cis_99$capture_mu))
```

```{r}
ggplot(aes(x=bounds, y=id, color=capture_mu, group=id), data=cis_99_df) +
  geom_point(size=2) +
  geom_line() +
