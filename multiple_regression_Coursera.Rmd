...
ggplot(aes(x=age, y=bty_avg), data=evals) + geom_point() + stat_smooth(method='lm', se=FALSE)
```

```{r}
ggplot(aes(x=gender, y=bty_avg), data=evals) + geom_boxplot()
```
...
```{r jitter}
# type your code for the Exercise here, and Knit
ggplot(aes(x=bty_avg, y=score), data=evals)+geom_jitter()
```
...
ggplot(aes(x=gender, y=score), data=evals) + geom_point()
```
