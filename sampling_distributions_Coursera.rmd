...
```{r}
ggplot(aes(x=area), data=ames) + geom_histogram(binwidth = 250)
```
...
```{r}
samp1 %>% 
  summarise(samp1_avg=mean(area), samp1_med=median(area), samp1_sd=sd(area), 
...
```{r}
sample_means_small <- ames %>% rep_sample_n(size=10, rep=25, replace=TRUE) %>% 
  summarise(x_bar=mean(area))
ggplot(aes(x=x_bar), data=sample_means_small) + geom_histogram(bins=20)
```
...
```{r}
ames %>% summarise(mu=mean(price), pop_med=median(price), sigma=sd(price), pop_iqr=IQR(price), pop_min=min(price), pop_max=max(price), pop_q1=quantile(price, 0.25), pop_q3=quantile(price, 0.75))
```

```{r}
ggplot(aes(x=price), data=ames) + geom_histogram(bins=30)
```

```{r}
samp_price1 <- ames %>% sample_n(size=50)
```

```{r}
samp_price1 %>% summarise(x_bar=mean(price))
```

```{r}
ggplot(aes(x=price), data=samp_price1) + geom_histogram(bins=10)
```
...
ggplot(aes(x=x_bar), data=sample_means50) + geom_histogram(bins = 30)
```
...
sample_means150 <- ames %>% 
  ...
  summarise(x_bar = mean(price))
ggplot(aes(x=x_bar), data=sample_means150) + geom_histogram(bins=30)
...
```{r}
ames %>% sample_n(size=15) %>% summarise(mean=mean(price))
```
...
sample_means15 <- ames %>% rep_sample_n(size=15, reps=2000, replace=TRUE) %>% 
  summarise(x_bar=mean(price))
ggplot(aes(x=x_bar), data=sample_means15) + geom_histogram(bins=50)
...
sample_means150 <- ames %>% 
  ...
  summarise(x_bar=mean(price))
ggplot(aes(x=x_bar), data=sample_means150) + geom_histogram(bins=50)
summary(sample_means150)  
```
...
```{r}
IQR(sample_means50$x_bar)
IQR(sample_means150$x_bar)
```
